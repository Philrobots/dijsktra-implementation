...
unsigned int Graphe::plusCourtChemin(size_t p_origine, size_t p_destination, std::vector<size_t> &p_chemin) const {
        ...
        // faire un typedef de pair d'intéger
        typedef pair<unsigned int, unsigned int> sommetsEtPoids;

        // creer la file de priorité ou on met les sommets qui sont en train d'etre traités
        priority_queue<sommetsEtPoids , vector<sommetsEtPoids>, greater<sommetsEtPoids>> filePriorite;

        // on rajoute dans la file le sommet source avec une distance de 0
        filePriorite.push(make_pair(zero, p_origine));
        distance[p_origine] = zero;

        // on boucle jusqu'à temps que la file deviennent devienne vide
        while (!filePriorite.empty()) {

            // le premier sommet de la pair c'est lui avec la distance la plus petite , il faut l'extraire
            unsigned int sommetDistanceMinimum = filePriorite.top().second;
            filePriorite.pop();

            solutionne[sommetDistanceMinimum] = true;

            if (sommetDistanceMinimum == p_destination) {
                break;
            }

            // boucle sur tout les sommets adjacents du sommet avec la distance minimal
            for (list<Graphe::Arc, std::allocator<Graphe::Arc>>::const_iterator arc = m_listesAdj[sommetDistanceMinimum].begin(); arc != m_listesAdj[sommetDistanceMinimum].end(); ++arc) {
                // aller chercher la destination et le poids du sommet adjacent
                unsigned int poidsDeArc = arc->poids;
                size_t sommetDestination = arc->destination;

                // on verifie que le sommet n'a pas été solutionné
                if (!solutionne[sommetDestination]) {
                    // s'il a une distance plus petite vers la destination
                    if (distance[sommetDistanceMinimum] + poidsDeArc < distance[sommetDestination] ) {

                        // on met à jour le nouveau chemin pour aller a la destination du sommet
                        distance[sommetDestination] = distance[sommetDistanceMinimum] + poidsDeArc;
                        predecesseur[sommetDestination] = sommetDistanceMinimum;
                        filePriorite.push(make_pair(distance[sommetDestination], sommetDestination));
                    }
                }
            }
        }
        // on garde le reste comme avant
...
/* algorithme de Dijkstra de base
//! \brief Version amméliorée de l'algorithme de Dijkstra permettant de trouver le plus court chemin entre p_origine et p_destination
//! \pre p_origine et p_destination doivent être des sommets du graphe
//! \return la longueur du plus court chemin est retournée
//! \param[out] le chemin est retourné (un seul noeud si p_destination == p_origine ou si p_destination est inatteignable)
